...
PVideoFrame __stdcall TCPClient::GetFrame(int n, IScriptEnvironment* env) {
    ...
    if (client->reply->last_reply_type == SERVER_SENDING_FRAME) {
        ...
        if ((unsigned int)dst->GetRowSize() != fi->row_size[0])
                env->ThrowError("TCPClient: Internal Error - rowsize alignment was not correct.");
        if ((unsigned int)dst->GetHeight() != fi->height[0])
        ...
        uint8_t* srcp = (unsigned char*)client->reply->last_reply + sizeof(ServerFrameInfo);
        TCPCompression* t = 0;
    ...
    _RPT0(0, "TCPClient: Pre unpack.\n");
    t->componentsize = vi.ComponentSize(); // DJATOM: we must know this, otherwise DeltaDecode will fail. Can be wrapped with "if (fi->compression != ServerFrameInfo::COMPRESSION_NONE)" since we don't use DeltaEncode/Decode in that case.
    ...
    int planeoffcet = 0;
    _RPT0(0, "TCPClient: pre-loop.\n");
    for (int pid = 0; pid < planecount; pid++) {
        _RPT1(0, "TCPClient: begin loop %d.\n", pid);
        int plane = vi.IsPlanar() ? planes[pid] : 0;
        t->DeCompressImage(srcp, fi->row_size[pid], fi->height[pid], fi->pitch[pid], fi->comp_plane_size[pid]);
        env->BitBlt(dst->GetWritePtr(plane), dst->GetPitch(plane), t->dst, fi->pitch[pid], dst->GetRowSize(plane), dst->GetHeight(plane));
        if (!t->inplace) _aligned_free(t->dst);
        if(planecount > 1)
            srcp += fi->comp_plane_size[pid];
        _RPT1(0, "TCPClient: end loop %d.\n", pid);
    }
    delete t;
    _RPT0(0, "TCPClient: post-loop.\n");
    } else {
    ...
    if (true) { // Request next frame
        f.n = n + 1;
        client->SendRequest(CLIENT_REQUEST_FRAME, &f, sizeof(ClientRequestFrame));
        _RPT1(0, "TCPClient: PreRequesting frame %d.\n", f.n);
    }
...
unsigned __stdcall StartClient(void* p) {
...
TCPClientThread::TCPClientThread(const char* hostname, int port, const char* compression, IScriptEnvironment* env) {
    ...
    _beginthreadex(NULL, 0, StartClient, this, 0, NULL);
    ...
    if (reply->last_reply_type != REQUEST_CONNECTIONACCEPTED) {
        env->ThrowError("TCPSource: Version Check failed!\n(Ensure Client and Server are same version)");
    }
}
...
void TCPClientThread::StartRequestLoop() {
    ...
    delete[] reply->last_reply; // DJATOM: crashing here when 'major' version mismatch occurred. Caught with AvspMod, but not happening with avs2yuv.
        reply->last_reply = 0;
