...
#define TCPDELIVER_MAJOR 4 // DJATOM: bumped since 0.2
...
struct ServerFrameInfo {
    ...
    unsigned int row_size[4]; // The array with width of a used pixels of a line in bytes.
    unsigned int height[4]; // The array with height of the image in pixels 
    unsigned int pitch[4]; // The array with length of a line in bytes.
    ...
    unsigned int comp_plane_size[4]; // The array with compressed size of planes.
    enum {
        COMPRESSION_NONE = 0, // The image sent as is.
