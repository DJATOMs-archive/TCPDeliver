...
public:
    ...
    virtual int CompressImage(uint8_t* image, int rowsize, int h, int pitch); // returns new size
    virtual int DeCompressImage(uint8_t* image, int rowsize, int h, int pitch, int data_size); // returns new size

    uint8_t* dst; // Must be deallocated using _aligned_free().
    int compression_type;
    int componentsize = 1;
    bool inplace; // Do NOT free dst, when true.
    template<typename pixel_t> void DeltaEncode(uint8_t * image, int pitch, int rowsize, int h);
    template<typename pixel_t> void DeltaDecode(uint8_t * image, int pitch, int rowsize, int h);
};
...
public:
    ...
    int CompressImage(uint8_t* image, int rowsize, int h, int pitch);
    int DeCompressImage(uint8_t* image, int rowsize, int h, int pitch, int data_size);
private:
    lzo_bytep wrkmem;
