...
unsigned __stdcall StartServer(void* p) {
...
TCPServerListener::TCPServerListener(int port, PClip _child, IScriptEnvironment* _env) : child(_child), env(_env) {
    ...
    if (bind(m_socket, (SOCKADDR*) &service, sizeof(service)) == SOCKET_ERROR) { // DJATOM: that's a place where TCPServer fails when no MtMode specified.
    ...
    shutdown = false;
    _beginthreadex(NULL, 0, StartServer, this, 0, NULL);
    thread_running = true;
}
...
void TCPServerListener::Listen() {
    ...
    while (!shutdown) {
        ...
        if (!request_handled) {
            ...
            if (prefetch_frame > 0) {
                _RPT1(0, "TCPServer: Prerequesting frame: %d.\n", prefetch_frame);
...
void TCPServerListener::Receive(TCPRecievePacket* tr, ServerReply* s) {
    ...
    case CLIENT_CHECK_VERSION:
        ...
        break;
    default:
        _RPT1(1, "TCPServer: Could not handle request type %d.\n", recvbuf[0]);
        tr->isDisconnected = true;
        break;
    }
}
...
// Requests should be optimally handled by separate thread to avoid blocking other clients while requesting the frame.
void TCPServerListener::SendFrameInfo(ServerReply* s, const char* request) {
    ...
    s->client->compression->componentsize = vi.ComponentSize();

    int planes_y[4] = {PLANAR_Y, PLANAR_U, PLANAR_V, PLANAR_A};
    int planes_r[4] = {PLANAR_G, PLANAR_B, PLANAR_R, PLANAR_A};
    int *planes = (vi.IsPlanarRGB() || vi.IsPlanarRGBA()) ? planes_r : planes_y; // according to pinterf's code in the Average.

    bool hasAlpha = vi.IsPlanarRGBA() || vi.IsYUVA();
    int planecount = (vi.IsY() || !vi.IsPlanar() ? 1 : (hasAlpha ? 4 : 3)); // DJATOM: Non-planar cases will be handled properly... I think.

    uint8_t* dstp;
    uint8_t* dst[4] = {}; 

    for (int pid = 0; pid < planecount; pid++) {
        int plane = vi.IsPlanar() ? planes[pid] : 0;
        sfi.height[pid] = src->GetHeight(plane);
        sfi.row_size[pid] = src->GetRowSize(plane);
        sfi.pitch[pid] = src->GetPitch(plane);
        sfi.comp_plane_size[pid] = s->client->compression->CompressImage(src->GetWritePtr(plane), src->GetRowSize(plane), src->GetHeight(plane), src->GetPitch(plane));
        sfi.compressed_bytes += sfi.comp_plane_size[pid];
        dst[pid] = s->client->compression->dst;
    }
    ...
    for (int pid = 0; pid < planecount; pid++) {
        ...
        sfi.data_size += src->GetHeight(plane) * src->GetPitch(plane);
        if (pid < 1) {
            env->BitBlt(dstp, 0, dst[pid], 0, sfi.comp_plane_size[pid], 1);
        } else {
            env->BitBlt(&dstp[planeoffcet], 0, dst[pid], 0, sfi.comp_plane_size[pid], 1);
        }
        planeoffcet += sfi.comp_plane_size[pid];
        if (!s->client->compression->inplace) {
            _aligned_free(dst[pid]);
        }
    }
...
TCPRecievePacket::TCPRecievePacket(SOCKET _s) : dataSize(0), data(0), isDisconnected(false), s(_s) {
    int recieved = 0;
    while (recieved < 4) {
